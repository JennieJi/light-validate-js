...
var invalidWithErrorMessage = function() {
  return errorMessage;
};
var validatorWithParam = function(value, isValid) {
  return isValid;
};
var validGroup = [
    [valid],
  ...
  invalidGroup = [[valid], [invalid], [validatorWithParam, false]],
...
var asyncInvalidGroup = [[asyncValid], [asyncInvalid], [asyncCatch]];
var asyncInvalidGroup2 = [[asyncValid], [asyncCatch], [asyncCatch]];
...
describe('Single Validate', function() {
  ...
  describe('Normal Validate', function() {
    describe('Valid', function() {
      it(
        '1 validator without param',
        assertValid(validate(testValue, [[valid]]))
      );
      it(
        '1 validator with param',
        assertValid(validate(testValue, [[validatorWithParam, true]]))
      );
      it(
        '1 validator by passing object',
        assertValid(
          validate(testValue, [
            {
      ...
      it(
        'Multiple validator without param',
        assertValid(validate(testValue, [[valid], [valid], [valid]]))
      );
      it(
        'Multiple validator with param',
        assertValid(validate(testValue, validGroup))
      );
    });
    describe('Invalid', function() {
      it(
        '1 validator without param',
        assertInvalid(validate(testValue, [[invalid]]))
      );
      it(
        '1 validator with param',
        assertInvalid(validate(testValue, [[validatorWithParam, false]]))
      );
      it(
        '1 validator by passing object',
        assertInvalid(
      ...
      it(
        '1 validator return error message directly',
        assertInvalid(validate(testValue, [[invalidWithErrorMessage]]))
      );
      it(
        'Multiple validator without param',
        assertInvalid(validate(testValue, [[valid], [invalid], [valid]]))
      );
      it(
        'Multiple validator with param',
        assertInvalid(validate(testValue, invalidGroup))
      );
      it(
        'Multiple validators which return error message directly by validator',
        assertInvalid(validate(testValue, invalidGroupReturnErrorMessage))
      );
    });
  });
  ...
  describe('Async Validate', function() {
    describe('Valid', function() {
      ...
      it(
        'Multiple validator',
        assertValid(validate(testValue, asyncValidGroup))
      );
    });
    describe('Invalid', function() {
      it(
        '1 validator and response 200 with false',
        assertInvalid(validate(testValue, [[asyncInvalid]]))
      );
      it(
        '1 validator and response error',
        assertInvalid(validate(testValue, [[asyncCatch]]))
      );
      it(
        'Multiple validator includes response 200 with false only',
        assertInvalid(
          validate(testValue, [[asyncValid], [asyncInvalid], [asyncValid]])
        )
      );
      it(
        'Multiple validator includes response error only',
        assertInvalid(
          validate(testValue, [[asyncValid], [asyncCatch], [asyncValid]])
        )
      );
      it(
        'Multiple validator includes 2 kinds of error responses',
        assertInvalid(validate(testValue, asyncInvalidGroup))
      );
      it(
        'Multiple validator includes 2 kinds of error responses',
        assertInvalid(
          validate(testValue, [[asyncValid], [asyncCatch], [asyncInvalid]])
        )
      );
    });
...
describe('Group Validate', function() {
  ...
  describe('Normal Validate', function() {
    ...
    describe('Invalid', function() {
      ...
      it(
        'Exit once error',
        assertGroupInvalid(groupValidate(group), ['field2'])
      );
      it(
        'Wait for all',
        assertGroupInvalid(groupValidate(group, false), ['field2', 'field3'])
      );
    });
  });
  ...
  describe('Contains Async Validate', function() {
    ...
    describe('Normal with catch', function() {
      ...
      it(
        'Exit once error',
        assertGroupInvalid(groupValidate(group2), ['field2'])
      );
      it(
        'Wait for all',
        assertGroupInvalid(groupValidate(group2, false), ['field2', 'field3'])
      );
    });
  });
});
